...
| 3 — Design implementation | 28–29 | ✅ **CLOSED — PR #8 (12 commits) merged to master `3665473` + James browser-verified 2026-07-07** (all-"new" rollup chips = correct 3-weeks-of-history state; trends unlock Jul 13 rates / Jul 20 counts; admin-config visual polish → PR #9 same day). Session 28 (2026-07-07, PR #7, 11 commits, MERGED `66c7b5c` + browser-verified): tokens + self-hosted fonts · ONE trend engine in shared · flags-only coaching engine · the inversion (`/scorecard/:employeeId?` home; DashboardPage/useDirectReports deleted) · Cadence chrome + **S1 CLOSED** · count-anchoring + jsdom decisions resolved. Session 29 (2026-07-07, PR #8, 11 commits): rollup reskin + its trend migration onto `metricTone` (**D6 FULLY CLOSED** — last pre-Cadence trend code gone) · SharedScorecardPage = the briefing's evidence view-model with per-row synced stamps (KpiTile deleted) · Cadence PDF (both windows + tone words, every-page watermark) · admin + login/callback reskin · **S10 + S12 CLOSED** · transitional aliases retired (grep-verified) · component tests for the remaining surfaces · pilot guide rewritten. Tests 79→142→**154** (50 api / 72 web / 32 shared). After the merge: the demo (`docs/demo-smoke-checklist.md`) |
...
| D6 | Trend computed 2 ways with different semantics — KpiTile badge (latest vs prior-average of history) vs rollup chip (thisWeek vs lastWeek) | `KpiTile.tsx:14-31` vs `useManagerRollup.ts:141-155` | ✅ **FULLY CLOSED Phase 3 session 29**: ONE definition (Cadence engine, `packages/shared/src/trend.ts` + `metricTone` in `lib/evidence.ts`) everywhere; the rollup chips were the last holdout and migrated with the rollup reskin; KpiTile itself is deleted |
...
| S10 | ~~No 404 route — `*` silently redirects, so typo'd deep links vanish without feedback~~ ✅ **CLOSED Phase 3 session 29 (2026-07-07)**: `*` renders a real Cadence 404 card (message + "Go to your team"); the `/` landing redirect is explicit | `App.tsx`, `NotFoundPage.tsx` | done |
...
| S12 | ~~Document title static across routes; nav lacks `aria-current`; route changes don't move focus~~ ✅ **CLOSED Phase 3** — aria-current landed with the session-28 chrome; session 29 added `useDocumentTitle` on every route + AppLayout focuses the page heading on page mount (not on same-page param changes) | app-wide | done |
