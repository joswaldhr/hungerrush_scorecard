...
| 2026-07-08 | Audit PR 3 (REVIEW.md 3.1 — ghost employees): `employees.is_active` (default true). Org-sync pass 5 reconciles against the full enabled-member email set — absent → `is_active=false` (rows are NEVER deleted), reappearing → back to true; circuit breaker skips deactivation if more than max(5, 20% of active rows) would flip at once (a thin Graph read must never mass-deactivate). UI badges inactive people "no longer synced" (roster chip, briefing header, rollup card count); rollup tone counts exclude their frozen histories | `0020_employees_is_active.sql` |
