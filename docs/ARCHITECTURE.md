...
- **Discovered 2026-08-28 while building roster auto-discovery (Phase 15), partially fixed 2026-08-31**: two-thirds of the seeded `external_identities` rows (24 of 36, for *both* Zendesk and Assembled) used `@revention.onmicrosoft.com` email addresses that matched zero real Zendesk agents (the real domain is `@hungerrush.com`). Since both connectors match tickets/activities by searching on `external_identities.externalId` as a literal email, every affected person had been silently getting no real ticket/activity data matched — not a sync error, just zero matches, which upstream code correctly renders as "no data" rather than a visible failure, so nothing looked broken.
  ...
  - **Historical Assembled finding, superseded by the September 25 census**: the August investigation could not find 21 people and left old external identity rows unchanged. On September 25 a complete deleted-inclusive API lookup returned 99 unique people and matched all 62 current employees uniquely by exact company email to nondeleted people. A bounded ticket-state/effort sample is available. This verifies current access and vendor roster coverage, not correctness of the old Cadence Assembled identity rows or production effort metrics. The connector remains removed/retired. See `docs/audits/2026-09-25-workforce-source-capability.json` and the authoritative implementation ledger.
