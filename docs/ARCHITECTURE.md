...
- **Urgent, discovered 2026-08-28 while building roster auto-discovery (Phase 15), not yet fixed**: two-thirds of the seeded `external_identities` rows (24 of 36, for *both* Zendesk and Assembled) use `@revention.onmicrosoft.com` email addresses. Confirmed directly against the live Zendesk account that this domain matches **zero** real agents — the actual live agents use `@hungerrush.com` (verified for at least one: `michael.vonhatten@hungerrush.com` exists and is active; `michael.vonhatten@revention.onmicrosoft.com` returns no results). Since both connectors match tickets/activities by searching on `external_identities.externalId` as a literal email, every person with a `@revention.onmicrosoft.com` identity has very likely been silently getting no real ticket/activity data matched this whole pilot — not a sync error, just zero matches, which upstream code correctly renders as "no data" rather than a visible failure, so nothing looked broken. This needs a decision from James before any fix: correct the 24 rows' `externalId` to the real `@hungerrush.com` equivalent (if that's uniformly the right mapping), or something else if the historical domain is intentional for some employees. Do not bulk-edit `external_identities` without that confirmation — this directly affects which real tickets get attributed to which real person.
