...
**Execution status (sprint 1, one PR at a time):** PR 1 `audit/admin-role-fix` → **MERGED `8f8228e` 2026-07-08** (0019 applied + verified; sweep executed: 279 → employee, audited, probe passed; serving sha confirmed). PR 2a `audit/sync-hardening` → **MERGED `a14c8ce` 2026-07-08** (serving sha confirmed). PR 2b `audit/toolchain-2026-07` → **MERGED `371b356` 2026-07-08** (npm audit 7 → **0**; serving sha confirmed; first post-deploy cron check pending at the next boundary). PR 3 `audit/ghost-reconciliation` → **MERGED `90f73c2` 2026-07-08** (0020 applied — first paste succeeded, the 42701 was a double-run; serving sha confirmed). PR 4 `audit/contrast-tokens` → **MERGED `f3b0d25` 2026-07-08** (James's visual pass owns the verdict per decision 1). **PR 5 `audit/qol-sprint1` → NOT STARTED — next session begins here**: unsaved-note guard (dirty check + confirm on person-switch + beforeunload), optimistic action-item toggles (rollback + WarnBanner copy, `useScorecardNotes.ts` toggle), keyboard basics (`/` focuses roster search, `←/→` person nav, `Esc` clears — plain listeners, never while typing in inputs), freshness chips (rollup subtitle + global header `synced X ago`, amber past the 9h bound in `evidence.ts`). Gate: tests green + tests for the dirty-guard and optimistic-rollback paths. **Pending checks:** first node-cron-v4 production cron (18:00 UTC 2026-07-08 — verify a `sync_run` audit row landed ~18:07); Supabase session survival after AD-disable (IT, open question 2).
