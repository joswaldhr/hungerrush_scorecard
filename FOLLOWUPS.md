...
| 29 | *(new, 2026-09-23)* Scorecard print layout's one-page margin is shrinking as content is added | Not a bug yet — measured 877-960px against a ~979px conservative budget after this session's additions (status-explanation line, section icons); was 877px before them | No action now, but re-measure (see `scripts` or the live-CSS-extraction technique in this session's commits) before adding more content to the scorecard header or category cards, or it will silently spill to a 2nd page again |
