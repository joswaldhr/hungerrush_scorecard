...
import { csatPolicyForPeriod } from "@/lib/connectors/zendesk-csat-policy";
import { createCsatConnector } from "@/lib/connectors/zendesk-csat-connector";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    logger.error("CSAT scheduled sync failed", { error });
    return NextResponse.json(
      { error: "CSAT sync failed; check source diagnostics" },
      { status: 503 }
    );
  }
}
