...
it("requires source/account/organization ownership before resolving active employees", async () => {
  expect(await loadFirstReplyEmployeeBindings(policy, config, "2026-09-20")).toEqual([
  ...
  try {
    await expect(loadFirstReplyEmployeeBindings(policy, config, "2026-09-20")).rejects.toThrow(
      /binding/
    );
  } finally {
    await db
...
it("rejects incompatible duration definition units before fetching", async () => {
  await db.update(metricDefinitions).set({ unit: "s" }).where(eq(metricDefinitions.id, score));
  try {
    await expect(loadFirstReplyEmployeeBindings(policy, config, "2026-09-20")).rejects.toThrow(
      /business-minute/
    );
